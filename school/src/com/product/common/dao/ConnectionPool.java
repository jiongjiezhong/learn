...
public class ConnectionPool {
	...
	/** 存放连接池中数据库连接的向量,它中存放的对象为PooledConnection型 */
	...
	public synchronized PooledConnection getConnection() throws SQLException {
		...
		while (conn == null) {
			...
			// 等一会再试
			wait(250);
			// 重新再试，直到获得可用的连接，如果getFreeConnection() 返回的为 null则表明创建一批连接后也不可获得可用连接
	...
	private void closeConnection(Connection conn) {
		...
		try {
			...
			log.error(" 关闭数据库连接出错： " + e.getLocalizedMessage());
