...
import com.product.common.dao.MultipartFileDao;
...
import com.product.utils.MultipartFileUtil;
...
public class TblUserServiceImpl implements TblUserService {
	 ...
	 * @return
	 */
	@Cacheable(key = "#userId", value = "TblUser")
	@Override
	...
	private void uploadPicUpdate(TblUser user) {
		if (ObjectUtil.isFileNotEmpty(user.getAllMultiFile())) {
			// 处理用户图片
			MultipartFileUtil.updateAllMulti(user, WanmaConstants.USER_NORMALUSE,
					user.getUserId() + "");
		}
		if (ObjectUtil.isFileNotEmpty(user.getIdCardPic())) {
			// 处理个体商家身份证
	...
	public String findPhoneByAccount(TblUser user) {
		...
		user = tblUserMapper.getAllUserByAccount(user);
		if(user == null){
			dwzResult = new DwzAjaxResult("300", "账号错误，请重新输入", "", "", "");
			return new JsonObject(dwzResult).toString();
		}else if(user.getUserLevel() == 6){
			dwzResult = new DwzAjaxResult("300", "该手机号非后台用户，请通过app或web端找回", "", "", "");
			return new JsonObject(dwzResult).toString();
		}else {
			if(WanmaConstants.USER_STATUS_FROZEN == user.getUserStatus()){
				dwzResult = new DwzAjaxResult("300", "账号被冻结，请联系管理员", "", "", "");
			}else if(WanmaConstants.USER_STATUS_DELETE== user.getUserStatus()){
				dwzResult = new DwzAjaxResult("300", "账号已删除，请联系管理员", "", "", "");
			}else{
				TblUser resetUser = tblUserMapper.findPhoneByAccount(user);
				if(resetUser.getPhone() == null || "".equals(resetUser.getPhone()))
					dwzResult = new DwzAjaxResult("300", "手机号码不存在，请联系管理员", "", "", "");
				else
					dwzResult = new DwzAjaxResult("200", "", "", resetUser.getPhone() + ","
							+ user.getUserAccount()+ "," + resetUser.getUserLevel(), "", "");
			}
			return new JsonObject(dwzResult).toString();
		}
	}
