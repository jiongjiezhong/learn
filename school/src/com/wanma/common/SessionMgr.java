...
import com.product.common.WebConst;
...
public class SessionMgr {
    ...
    public static TblUser getWebUser(HttpServletRequest request) {
        return (TblUser) request.getSession().getAttribute(WebConst.SESS_PRINCIPAL);
    }
    ...
    public static void addWebUser(HttpServletRequest request, TblUser user) {
        if(user==null) return;
        request.getSession().setAttribute(WebConst.SESS_PRINCIPAL, user);
    ...
    public static boolean removeWebUser(HttpServletRequest request) {
        ...
        session.removeAttribute(WebConst.SESS_PRINCIPAL);
        session.invalidate();
        return true;
    }
