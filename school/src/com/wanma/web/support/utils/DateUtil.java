...
public class DateUtil {
	 ...
	 *         </p>
	 */
	public static int compareToCurTime(Date paramDate) {
		if (paramDate == null) {
			return -2;
		}
		Date currentDate = new Date();
		Calendar calDateA = Calendar.getInstance();
		calDateA.setTime(currentDate);
		Calendar calDateB = Calendar.getInstance();
		calDateB.setTime(paramDate);
		// 比较年份小于当前年份
		if (calDateB.get(Calendar.YEAR) < calDateA.get(Calendar.YEAR)) {
			return -1;
		}
		// 比较年份等于当前年份
		if (calDateB.get(Calendar.YEAR) == calDateA.get(Calendar.YEAR)) {
			// 比较月
			if (calDateB.get(Calendar.MONTH) < calDateA.get(Calendar.MONTH)) {
				return -1;
			}

			if (calDateB.get(Calendar.MONTH) == calDateA.get(Calendar.MONTH)) {
				// 比较日
				if (calDateB.get(Calendar.DAY_OF_MONTH) < calDateA
						.get(Calendar.DAY_OF_MONTH)) {
					return -1;
				}
	...
	public static final Date formatDate(Date srcDate, String dMask)
			throws ParseException {
		SimpleDateFormat df = null;
		Date date = null;
		df = new SimpleDateFormat(dMask);

		if (logger.isDebugEnabled()) {
			logger.debug("converting '" + dMask + "' to date with mask '"
					+ dMask + "'");
		}
		try {
			date = df.parse(dMask);
	...
	/**
	 * 时间戳转换指定时间格式 yyyy-MM-dd HH:mm:ss
	 *
	 * @param timestamp
	 * @return
	 */
	public static String convertTimestampToDateStr(long timestampParam) {
		Timestamp timestamp = new Timestamp(timestampParam);
	 ...
	 * @throws java.text.ParseException
	 */
	public static String[] getStringArrDate(String start, String end)
			throws ParseException {
		...
		while (startDate.before(endDate)) {
			dateStr.add(sdf.format(startDate));
			startCalendar.setTime(startDate);
			startCalendar.add(Calendar.DATE, 1);
			startDate = startCalendar.getTime();
		}

		String[] strArr = new String[dateStr.size()];
		dateStr.toArray(strArr);
		return strArr;
	}
