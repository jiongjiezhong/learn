		        ...
		        success: function(datas){
		       	 if(datas.pager.code=="OK"){
		       		 ...
		       		 for(var i =0;i<data.length;i++){
		       			 ...
		       			 for(var j=k;j<replyData.length;j++){
		       				 if(replyData[j].up_commentId == commentId){
		       					//获取回复信息
		       					replyComment +='<div id="centerLinkReplyDiv'+replyData[j].pk_PsComment+'" class="PLleft PL-HF"><img src="'+replyData[j].userImage+'" onerror="javascript:this.src=\''+basepath+'/static/images/prettyPhoto/user_b.png\'" width="80" height="80" /></div>'+
		                ...
		                '</ul></div>';
		       			...
		       			//评论内容
				       	 comments +='<div class="PLleft"><img src="'+data[i].userImage+'" onerror="javascript:this.src=\''+basepath+'/static/images/prettyPhoto/user_b.png\'" width="80" height="80" /></div>'+
			       			'<div class="PLright">'+
			       	        '<dl class="PL">'+
		       ...
		       success: function(datas){			        
		        	if(datas.code=="OK"){
				    	   ...
				    	   $('#zhuangreplyDiv'+commentId).attr('data_reply_count',newReply_count);
		  			       ...
		  			       //动态增加回复信息
				    	   userImgUrl = $('#userImgDiv').attr('src')
				    	   $('#replyDetailDiv'+commentId).prepend('<div class="PLleft PL-HF"><img src="'+userImgUrl+'" onerror="javascript:this.src=\''+basepath+'/static/images/prettyPhoto/user_b.png\'" width="80" height="80" /></div>'+
				  					'<div class="PLright PL-HFRight">'+
				  			        '<dl class="PL">'+
				  			        '<dt>'+user.usinUsername+'</dt>'+
		  	...
		  	$.ajax({
		        ...
		        data:{
		          ...
		          psc_Content:powerStationContent,
		  		  psc_UserId:user.pkUserinfo,
		  		  psc_UserName:user.usinUsername
		        },
		        success: function(datas){
		        	if(datas.code=="OK"){
		        	$("#pingfen-neirong").val('');
	  ...
	  function addNewStationComment(user,data){
		  var datas = data.split("|||");
		   ...
		   var commentsDiv = '<div id="zhuangreplyDiv'+datas[1]+'" data_reply_count="0" class="BoxInput" style="display:none"><ul>'+
		                ...
		                '<span class="userNav"><img class="userImg" src="'+userImgUrl+'" width="27" height="27" alt=" " /><t class="userName">'+user.usinUsername+'</t></span></li>'+
		                '</ul></div>';
		   $("#tag").prepend('<div class="PLleft"><img src="'+userImgUrl+'" onerror="javascript:this.src=\''+basepath+'/static/images/prettyPhoto/user_b.png\'" width="80" height="80" /></div>'+
	       			'<div class="PLright">'+
	       	        '<dl class="PL">'+
	       	        '<dt>'+user.usinUsername+'</dt>'+
				       	 ...
				       	 if(datas.pager.code=="OK"){
				       		 ...
				       		 for(var i =0;i<data.length;i++){
				       			 ...
				       			 for(var j=k;j<replyData.length;j++){
				       				 if(replyData[j].up_commentId == commentId){
				       					//生成回复信息
				       					replyComment +='<div id="centerLinkReplyDiv'+replyData[j].pk_EpcComment+'" class="PLleft PL-HF"><img src="'+replyData[j].userImage+'" onerror="javascript:this.src=\''+basepath+'/static/images/prettyPhoto/user_b.png\'" width="80" height="80" /></div>'+
				       					'<div class="PLright PL-HFRight">'+
				       			        '<dl class="PL">'+
			  ...
			  var  electricPileId = $("#electricPileId").val();
			  	$.ajax({
			        ...
			        data:{
			        	...
			        	epc_UserId:user.pkUserinfo,
			        	epc_UserName:user.usinUsername
					  },
			        success: function(datas){			        
			        	if(datas.code=="OK"){
			        		$(".liuyan").remove();
...
function openCurrentDetailTab(obj){
	var url=$(obj).attr("ref");
	navTab.openTab("electricCurrentDetail", url,{title:"实时详情"});
}	