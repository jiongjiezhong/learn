	...
	if(typeof options=="string"){
		...
		if(options.rule){
			...
			$.each(options,function(name,value) { 
				opts={rule:name,param:value.param,msg:value.msg};
				res=valid(opts,v);
				 if(!res.result){
					 return false;
				 }
...
function valid(options,v){
	var rule = options.rule;
	switch (rule) {
		case "mobile":
			return validInfo(options,isMobile(v));
		case "phone":
			return validInfo(options,isPhone(v));
		case "ip":
			return validInfo(options,isIP(v));
		case "port":
			return validInfo(options,isPort(v));
		case "email":
			return validInfo(options,isEmail(v));	
		case "money":
			return validInfo(options,isMoney(v));	
		case "required":
			return validInfo(options,isNotNull(v));	
		case "chinese":
			return validInfo(options,isChinaOrNumbOrLetter(v));		
		case "date":
			return validInfo(options,isDate(v,options.param));				
		case "integer":
			return validInfo(options,isInteger(v));	
		case "number":
			return validInfo(options,isNumber(v));	
		case "mobileOrPhone":
			return validInfo(options,isMobileOrPhone(v));
		case "password":
			return validInfo(options,isNumberOr_Letter(v));
		case "numberOrLetter":
			return validInfo(options,isNumberOrLetter(v));		
		case "minLength":
			var param=options.param?options.param:6;
			if(typeof param=="string"){
				param=parseInt(param);
			}
			return validInfo(options,v.length>=param);	
		case "maxLength":
			var param=options.param?options.param:20;
			if(typeof param=="string"){
				param=parseInt(param);
			}
			return validInfo(options,v.length<=param);
...
function validInfo(options,result){
	var rule = options.rule;
	return {
		rule:rule,
		result:result,
...
/*
 * 用途：检查输入字符串是否为空或者全部都是空格 输入：str 返回： 如果全是空返回false,否则返回true
 */
...
/*
 * 用途：检查输入电话号码是否正确 输入： s：字符串 返回： 如果通过验证返回true,否则返回false
 * 
 */
function isPhone(s) {
...
function isMobileOrPhone(s) {
	...
	var phoneReg = /^0\d{2,3}-?\d{7,8}$/;
	if (mobileReg.test(s)||phoneReg.test(s)) {
		return true;
	} else {
		return false;
	}
...
function validateForm(formId,array){
	var res=null;
	for(var i=0;i<array.length;i++){
		res=$("#"+formId+" input[name="+array[i].name+"]").validate(array[i].rule);
		if(!res.result){
			break;
		}
	}
